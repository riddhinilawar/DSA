...
class Tree
{
    public static float findMedian(Node root)
    {
        ...
        float count=count(root,list);
        //System.out.println(list);
        if(count%2==0f)
            return (float)(list.get(((int)count-1)/2)+list.get((int)count/2))/2f;
        return (float)list.get((int)count/2);
    }
    public static float count(Node root,ArrayList<Integer> list){
        if(root==null)
            return 0f;
        float left=count(root.left,list);
        list.add(root.data);
        float right=count(root.right,list);
