...
class Solution {
    ...
    void doTraversal(Node root, int k, List<Node> path, Set<Node> st) {
        if (root == null) 
            return;
...
class Solution {
    // Function to return count of nodes at a given distance from leaf nodes.
    boolean check(Node root, int k, int dist) {
        if (root == null)
            return false;
