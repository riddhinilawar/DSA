...
class Solution {
    long sumBitDifferences(int[] arr, int n) {
       //from 32 bits traverse last 17 bits because constraint 100000 is 17 bit 
       
       long ans=0l;
       int bitSet[]=new int[17];
       
       for(int i=0;i<arr.length;i++){
           for(int j=0;j<17;j++){
               if((arr[i]&(1<<j))!=0)bitSet[j]++;
           }
       }
       
       for(int i=0;i<17;i++){
           long setBit=bitSet[i];
           long unSetBit=n-bitSet[i];
           
           ans+=(2*unSetBit*setBit);
       }
