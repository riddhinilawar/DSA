...
class solve {
    ...
    public boolean solve(int curr,boolean graph[][] ,int color[],int n, int m){
        
        if(curr==n)
            return true;
            
        for(int i=1;i<=m;i++){
            if(isSafe(graph,color,i,curr)){
                color[curr]=i;
                if(solve(curr+1,graph,color,n,m)==true)    
                    return true;
                color[curr]=0;
            }
        }
    ...
    public boolean isSafe(boolean graph[][] ,int color[],int col,int curr){
