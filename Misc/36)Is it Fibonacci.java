...
class Solution {
    static long solve(int N, int K, ArrayList<Long> GeekNum) {
        ...
        int start_pointer=GeekNum.size()-1;
        for(int i=K;i>0;i--){
            temp+=GeekNum.get(start_pointer);
            start_pointer--;
        }
        start_pointer++;
        
        //System.out.println(temp+" "+start_pointer);
        
        int itr=N-K;
        while(itr>1){
            GeekNum.add(temp);
            temp=temp+GeekNum.get(GeekNum.size()-1)-GeekNum.get(start_pointer++);
            itr--;
        }
